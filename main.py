...
class MyLayout(Widget):
    ...
    def press(self):
        ...
        if 'hello' in task:
            ...
            os.startfile(os.path.join(mudir, songs[0]))

        elif 'record' in task:
            say("Recording voice for 5 seconds")
            self.ids.outputLabel.text = "Recording done"
            freq = 44100
            duration = 5
            ...
            joke1 = joke.split()
            joke2=[]
            for i in range(len(joke1)):
                joke2.append(joke1[i])
                joke2.append(' ')
                if i%6==0 and i!=0:
                    joke2.append('\n')
            joke = ''.join(joke2)
            self.ids.outputLabel.text =joke

        elif 'make a note' in task:
            say("Wait a second, let me take my pen")
            say('What do you want me to make a note of?')
            note = listen()
            fo = open('note.txt', 'w')
            fo.write(note)
            fo.close()
            self.ids.outputLabel.text = "Made a note of what you said in note.txt"
            ...
            self.ids.outputLabel.text = "Search result available..."
            searchItem = task.replace("wikipedia", "")
            searchResult = wikipedia.summary(searchItem, sentences =2)
            self.ids.outputLabel.text = searchResult
            say(searchResult)
            ...
            if x["cod"] != "404":
                ...
                self.ids.outputLabel.text ="Temperature = " + str(
